...
| PR-F KbAccessServiceImpl implements 新端口 | ✅ | `4df42a4` | +31/-3；diamond override 解决双父接口 default 冲突；顺手清 SysDeptDO 未使用 import |
| PR-G MetadataFilterBuilder 抽 bean | ✅ | `82ade22` | +108/-24；2 个新文件；KbAccessService 注入需保留（见下文） |
| PR-H AccessScope 贯通 | ✅ | `cfda1dd` | +49/-32；6 文件；未登录态从 null=通行改为 empty()=fail-closed（设计修复） |
| PR-I RetrievedChunk 扩字段 | ✅ | `039af5c` | +46/-5；5 文件跨 framework+infra-ai+bootstrap；BaiLianRerankClient 是 plan 未提到的第 4 个正位构造点 |
| PR-J AuthzPostProcessor + 配置校验 | ✅ | `fc28abc` | +162/-1；2 个新文件；order=0 fail-close 纵深防御 |
...
**第 4 个正位构造点：** plan 只列了 `MilvusRetrieverService:89` 的 3-arg `new RetrievedChunk(id, content, score)`。实际 grep 另发现 `infra-ai/BaiLianRerankClient.java:165` 也用 3-arg 构造，`@AllArgsConstructor` 扩容后编译失败。改为 builder 并从 `src` 传递 `kbId/securityLevel`，保持 rerank 结果仍带授权字段（防未来 `AuthzPostProcessor` 挪到 rerank 之后时丢授权）。

**跨模块编译顺序验证：** 单独 `mvn -pl bootstrap clean compile` 会因 `BaiLianRerankClient` 仍加载旧 framework 而失败；`mvn -pl bootstrap -am clean compile`（`-am` = also-make）强制重建 framework + infra-ai + bootstrap 全链，问题自然解决。plan 的 "`mvn -pl framework install` 再 `mvn -pl bootstrap clean compile`" 对同时改了 infra-ai 的情形不完整。

---

## PR-H 执行中发现的 plan 缺口

**知识库域未被触及：** plan step 2.3 验证点只要求 `rag/core/retrieve/` 下 `accessibleKbIds` 零命中，**不包括**知识库域（`knowledge/controller/*`、`knowledge/service/impl/*`、`knowledge/controller/request/KnowledgeBasePageRequest.java`、`rag/service/impl/RAGChatServiceImpl.java` 以外的用法）。这些地方仍通过 `KbAccessService.getAccessibleKbIds(userId)` 拿集合。PR-H 无意替换它们 —— 那属于"老调用点分批迁移"，将在未来专项 PR 里处理。

**`RAGChatServiceImpl` 的 `kbAccessService` 字段保留：** 仅新增 `kbReadAccess` 不删旧字段；`checkAccess`、`isSuperAdmin` 等调用仍走老接口，与计划一致。

**`MultiChannelRetrievalEngine` 的 `kbAccessService` 也保留：** `buildSearchContext` 里 `getMaxSecurityLevelsForKbs` 仍通过老接口调用 —— 保持跨接口迁移与 AccessScope 贯通两件事解耦，commit diff 更小。如后续统一用 `KbReadAccessPort`，切换是一行改动。

---

## PR-G 执行中发现的 plan 缺口

**不要删 `KbAccessService` 注入：** plan step 1.6 说在 `MultiChannelRetrievalEngine` 里"删除 KbAccessService 注入（因为已不再调用 static 方法）"。实际该字段在 `buildSearchContext`（line 259）调用 `kbAccessService.getMaxSecurityLevelsForKbs(...)` 做 per-KB 安全等级预解析，与 static 方法无关，**必须保留**。

**`MultiChannelRetrievalEngine` 只有 1 处 static 调用不是 2 处：** plan 说"第 93 行和第 181 行"，实际该文件只有 line 93 的自调用，line 181 是 `VectorGlobalSearchChannel` 的跨文件调用。

**`ArrayList` import 顺手清：** 删 static 方法后该 import 变 unused，本 PR 一并移除。`MetadataFilter` 与 `MultiChannelRetrievalEngine` 同包无 import，无需处理。

---

## PR-F 执行中发现的 plan 缺口

**Diamond 冲突：** plan step 1.5 只说"加 `@Override` 即可"，但 `KbAccessService.getAccessibleKbIds(String)` 和 `KbReadAccessPort.getAccessibleKbIds(String)` 都是 `default` 方法，concrete 类同时继承两者时 javac 报 "unrelated defaults"。

**解决：** impl 显式 override `getAccessibleKbIds(String userId)` 返回 `getAccessibleKbIds(userId, Permission.READ)`，保留旧语义；新端口的 deprecated default 仅作迁移期标记。

**后续 PR-H/以后删除旧 `KbAccessService.getAccessibleKbIds(String)` default 后可删此 override。**
