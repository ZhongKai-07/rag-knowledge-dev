...
用户提问 → RAGChatServiceImpl
 ├─ 检索 (RetrievalEngine)
 │   └─ 检索层新增方法: 返回 List<EnrichedChunk>（保留旧 RetrievedChunk 路径）
...
| PR | 范围 | 可见影响 |
|----|------|----------|
| PR1 | `EnrichedChunk` + OpenSearch/Milvus/PG 检索层回填 metadata（新方法） + `KnowledgeDocumentService.findMetaByIds` + `DocumentMetaSnapshot` | 无行为变化，零业务影响 |
