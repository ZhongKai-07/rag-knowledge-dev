...
| 风险类别 | 描述 | 影响等级 | 缓解措施 |
|---|---|---|---|
| **合规风险** | 数据出境 / 核心资产泄露 / 监管处罚 | 高 — 量化策略 / 客户数据 / 订单数据若意外进入 SaaS prompt，可能触发监管处罚 | § 7.3 红线规则 + 法务核对 § 7.4 开放清单 + 部门级"prompt 输入白名单" |
| **锁定风险** | License 与厂商深度绑定 | 中 — Copilot 与 GitHub 生态、Gemini 与 GCP 认证、Kiro 与 AWS 体系绑定，退出需成本 | 退出策略（见 § 6.3）+ 多供应商组合（组合 A 已包含 Copilot + Gemini，天然对冲单一锁定） |
| **成熟度风险** | Kiro GA 仅 5 个月，能力 / 区域 / 定价频繁变化 | 中（限试点池范围内）— 试点项目可能因功能不稳定而中断，spec-driven 流程尚无大规模验证 | 限制 Kiro 用于试点池（5~10 席位），主力部门不依赖；阶段 1 结束前不扩座 |
| **数据风险** | Prompt 上传厂商服务器，即使不训练也存在第三方暴露面 | 高 — 服务商被攻破 / 内鬼 / 司法调取均可能导致代码 / 策略信息外泄 | 红线规则（§ 7.3）+ 各部门"不输入哪些内容"清单 + 数据脱敏前置处理 |

#### 补充风险

除上述 4 类主要风险外，还需关注以下 3 类：

- **License 池滥用风险**：Standard 池化自助申请若无审核流程，可能导致 token 消耗超预算；建议设置部门申请审批节点，配合 § 6.4 KPI #2 的月度池使用率跟踪。
- **培训与采用风险**：业务 / 运营 / 产品经理等非技术部门的工具采用门槛较高，若未配套培训资源，License 实际利用率可能偏低（空耗预算）；需纳入阶段 2 推广计划。
- **审计追溯风险**：AI 工具调用记录是否需符合证监会 / 交易所审计要求尚不明确；法务核对 § 7.4 开放清单时须专项确认，审计日志覆盖率纳入 § 6.4 KPI #4 跟踪。

---

### 6.2 三阶段落地路线图

> 以下时间线为**建议性**，不承诺硬性日期——实际节奏由公司管理层根据业务节奏调整。每个阶段设置明确的进入门槛，未达门槛不应推进到下一阶段。

```
阶段 1：试点（建议 4~6 周）
├─ 选 1~2 个部门做小规模试点：研发 + 算法各 10~20 人
...
- 三阶段时间线为建议性——实际节奏由公司管理层根据业务节奏调整，不代表对任何交付日期的承诺。
- 关键里程碑：法务核对完成（阶段 1 前置）、试点报告通过（阶段 1 → 2 门槛）、红线触发归零（全程监控）。
- "Kiro 试点池"的 3 个月评估窗口建议与阶段 1 时间**错开**：Kiro 试点可在阶段 2 初启动（法务核对 / SSO 接入已完成），避免在阶段 1 与核心法务 / IT 资源争夺带宽。
- 非技术部门（运营 / PM）建议在阶段 3 而非阶段 2 纳入，以确保培训资源充分准备。

---

### 6.3 退出策略

> 退出策略描述"如果工具不达预期如何切换"，迁移成本用**轻 / 中 / 重**标注。

#### Copilot Enterprise 退出

| 维度 | 说明 |
|---|---|
| **触发条件** | 模型质量下降 / 价格大幅上调 / 合规问题暴露（如 CMEK 信息缺口 #2 无法解决） |
| **迁移路径** | 切换到 Gemini Code Assist Enterprise（同为 IDE 插件架构，功能覆盖面相近） |
| **SSO / SCIM 重新对接** | 中——需重新配置 Google Workspace / OIDC 集成，估计 1~2 周 |
| **用户培训** | 轻——IDE 插件切换 + UI 习惯调整，Gemini Plugin 学习曲线平缓 |
| **GitHub Spaces 数据迁移** | 重——Copilot 的 GitHub 生态绑定（Codespaces、Actions Workflow、PR 集成）需手动或脚本化重建；与 GitHub 本身的绑定成本和工具无关，是平台级依赖 |
| **预期切换周期** | 6~8 周 |

#### Gemini Code Assist 退出

| 维度 | 说明 |
|---|---|
| **触发条件** | 模型质量下降 / 价格大幅上调 / GCP 生态绑定成本超预期 |
| **迁移路径** | 切换到 Copilot Enterprise（如已购）；或回退到自管 BYOK 模型 + 第三方 IDE 插件 |
| **长上下文工作流调整** | 中——Gemini 1M context 工作流（算法部门 PR review、长文档分析）需调整；Copilot 模型选单虽宽，但实际可用上下文受 harness 影响，工作流需重设计 |
| **GCP 生态解绑** | 重——算法部门若已深度使用 Vertex AI + Gemini Code Assist Enterprise 的 GCP 联动（Cloud Logging、CMEK、Cloud Workstations），解绑成本高；需提前评估依赖深度 |
| **用户培训** | 轻——切换到 Copilot 的 IDE 体验调整 |
| **预期切换周期** | 8~12 周 |
...
| 维度 | 说明 |
|---|---|
| **触发条件** | 试点未达预期 / GA 满 1 年仍有显著缺陷 / Kiro DPA 法务核对失败 |
| **迁移路径** | 因仅限试点 / 仅研发部门 / 不存关键资产，**退出成本最低**，直接回退到 IntelliJ / VSCode + Copilot/Gemini |
| **spec 文档迁移** | 中——spec-driven 流程产物（需求规格文档、spec 文件）格式为 Markdown，可迁移；但 Kiro 的"spec → task → code"自动化链路无法在其他工具直接复现，需调整工作流 |
| **团队习惯调整** | 轻——Kiro 试点期短，团队已有 Copilot/Gemini 使用经验，回退无障碍 |
| **预期切换周期** | 2~4 周 |

**退出策略总结**：三款工具中，Kiro 退出成本最低（试点性质、无深度依赖）；Gemini 退出成本最高（GCP 生态绑定可能较深）；Copilot 退出成本中等（GitHub 平台绑定是主要成本，与工具本身解绑相对容易）。组合 A 选择 Copilot + Gemini 双主力，本身已形成相互对冲——任一主力出现问题，另一家可作承接方。

---

### 6.4 风险监控指标（KPI）

建议每月跟踪以下 6 个 KPI，作为工具治理的量化依据，并作为 § 5.4 半年复审的输入数据：

| # | KPI | 目标值 | 说明 |
|---|---|---|---|
| 1 | **红线触发次数** | 0（任何 1 次触发均需法务介入） | 对应 § 7.3 红线规则；触发后需完成事后审查、记录根因、更新 prompt 白名单 |
| 2 | **License 池使用率** | Standard 池化 70~85% | 过高（> 90%）考虑扩容；过低（< 50%）考虑回收空闲席位；按部门分组跟踪 |
| 3 | **Token 消耗 vs 预算** | 按部门月度对账，偏差 ≤ 15% | 识别异常消耗（可能是滥用或工具误用）；Enterprise 级别均提供 Admin Console 视图 |
| 4 | **审计日志覆盖率** | 100%（所有工具调用有日志记录） | 对应审计追溯风险；Copilot / Gemini 均支持，Kiro 须在阶段 1 法务核对时确认 |
| 5 | **用户活跃度（DAU/WAU）** | 试点期 ≥ 70% 周活；推广期 ≥ 60% 周活 | 反映工具实际渗透率；低活跃度早期预警（培训不足 / 工具体验不匹配） |
| 6 | **生产力增益自报数据** | 每季度问卷，目标满意度 ≥ 4/5 | 作为半年复审的主观输入，配合 token 消耗等客观数据交叉验证；避免单一维度评估 |

> **KPI 跟踪节奏**：KPI #1（红线）实时监控 + 即时通报；KPI #2 / #3 / #4 每月对账（第一个工作周出报告）；KPI #5 每两周统计；KPI #6 每季度问卷。半年复审时汇总全部 6 项数据形成综合评估报告，作为"续约 / 调整 / 退出"决策依据。
