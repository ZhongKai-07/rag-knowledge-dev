...
**画像**：后端 / 前端 / 全栈 + SRE / DevOps；大型系统长期维护（50 万行以上）、超大型代码库生命周期比较长、跨模块重构、Spec-driven、CR、Bug 修复、CI/CD 脚本

**关键诉求**：仓库级上下文理解深度、多文件 Agent 编排、稳定 JetBrains / VS Code 集成、跨会话 Memory、Git/PR 工作流深度集成
...
**首选 Gemini Code Assist Enterprise** — 1M token 是核心优势（处理论文级 prompt + 多 Notebook 跨文件分析）/ Gemini Pro 数学 / 代码竞争力强 / GCP 生态深度集成（Cloud Workstations / BigQuery）/ Agent Mode 支持 MCP Server 接入 HuggingFace / W&B（需实测）；算法任务短平快，harness 落后（§ 3.1 评级**弱**）的短板相对可接受
...
**厂商定位**：Google Cloud 企业级 AI 编程助手，底层 Gemini，主打 1M token 长上下文和 Enterprise 私有代码库定制，与 GCP 生态深度绑定。
...
|---|---|---|---|
| 默认模型 | GPT5.X | Gemini 3.X Pro | Claude 混用） |
| 可选模型 | 15+ 跨厂商（GPT-5.x / Claude Opus 4.x / Gemini 3.x / Grok 等）| 仅 Gemini 系列；版本切换未公开说明 | Opus4.6 / Sonnet 4.6 / Haiku 4.5；用户可手动切换 |
...
- **影响最大**：研发（大型项目, 多次重构， 长生命周期演进）→ Copilot Spaces 索引成熟度最高；量化（合规 vs 上下文张力）→ 建议非核心代码用候选工具配严格 Policy，策略代码用 Claude Code 探索式路线
...
- **Gemini Code Assist Standard**：5 个部门首选——"经济性 + 自然语言友好性"帕累托最优，$19/月 + 无状态服务低运维 + Gemini 自然语言→代码强表现三者叠加
