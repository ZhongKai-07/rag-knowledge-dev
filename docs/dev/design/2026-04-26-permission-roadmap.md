...
把"前端传 indexNames 后端直接查"改成"后端计算 effective scope，前端值仅作意图建议"。

**业务侧**（兑现 enterprise doc Phase 4）：
- `RagRequest` 字段：`List<String> indexNames` → `List<String> kbIds + ScopeMode { SELECTED_KB, ALL_AUTHORIZED }`
- 引入一等公民 `RetrievalScope { kbIds, kbMaxSecurityLevelMap, permission }`
- 后端流程：`allowedKbIds = port.getAccessibleKbIds(userId, READ)` → `effectiveKbIds = scopeResolver.resolve(request.kbIds, allowedKbIds)`
- 前端传值仅作为意图建议，**不作为授权依据**
- OpenSearch query DSL 强制带：`terms(metadata.kb_id) + range(metadata.security_level lte X) + term(metadata.status=published)`
...
- `RAGChatServiceImpl.streamChat` 内 sources gate / SourceCardBuilder / emitSources 子流程提取（P0 报告 P2 拆分候选）

**Commit 序列模板**：
```
PR4 — RagRequest 字段迁移 + RetrievalScopeBuilder
  commit 1: 新增 RetrievalScopeBuilder + RetrievalScope 类型
  commit 2: RagRequest 加 kbIds + scopeMode；indexNames @Deprecated
  commit 3: RAGChatServiceImpl.streamChat 改用 RetrievalScopeBuilder
  commit 4: 前端 ChatStore 改传 kbIds，indexNames 字段不再发送
  commit 5: indexNames 字段物理删除

PR5 — OpenSearch filter 强制 + retriever 注入清理
  commit 1: OpenSearchRetrieverService 强制带 terms(kb_id) + range(security_level)
  commit 2: MultiChannelRetrievalEngine + VectorGlobalSearchChannel 改用 KbMetadataReader
  commit 3: 添加 SecurityLevelFilterEnforcedTest 契约测试
```

**成功标准**：
```
RagRequest 不再含 indexNames 字段
OpenSearch query DSL 强制带 terms(metadata.kb_id) + range(security_level)
FICC_USER 无 OPS-COB 绑定时检索结果为空（不抛错）
StreamChatEventHandler 不再调用 UserContext.getUserId()
MultiChannelRetrievalEngine 不再注入 KnowledgeBaseMapper
```

**引用源**：
- enterprise doc Phase 4（把权限压进 RAG 检索链路）
- P0 报告 §"RAG 编排拆分候选"
- P0 报告 §"向量后端不变量"（OpenSearch filter 当前已支持，本阶段强制启用）
