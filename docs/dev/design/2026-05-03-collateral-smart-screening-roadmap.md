...
| Phase 2.5 | Docling 增强解析 / Layout 文档智能底座 | Parser Enhancement PR1-6 / PR-DOCINT | 否；MVP 不承诺扫描件 OCR，预留质量检测 / `OcrFallbackParser` 抽象 |
...
## 6. Phase 2.5：Docling 增强解析 / Layout 文档智能底座

目标：

为 born-digital PDF、复杂版式协议和表格型协议建立可复核的文本与版面坐标底座。这个阶段不产生字段答案，只保证后续 Phase 3/4 能把字段值追溯回页码、块、可选 bbox。

**MVP 收敛口径（2026-05-05）**：

- 入库模式显式路由：`CHUNK` / 基础解析默认走 Tika；`PIPELINE` / 增强解析走 Docling。
- Collateral KB 文档入库默认或强制走 `PIPELINE`，由后端策略兜底，不只依赖前端选择。
- 暂不做 text-layer 质量自动检测；不承诺 image-only / scanned PDF 的 OCR 质量。
- 预留 `ParseModePolicy` / `TextLayerQualityDetector` / `OcrFallbackParser` 抽象，后续再启用自动推荐、低文本密度检测、mixed PDF 按页 fallback。

为什么独立成阶段：

- Phase 2 是安全门 + 查询链路修正，不能被 OCR 引擎选型拖住；
- Phase 3 接真实协议字段时，如果输入包含复杂版式、表格或后续扫描件，没有 layout metadata 会导致漏抽、错页、证据不可复核；
- Phase 5 的 Sources UI 只能展示证据，不能补造 OCR/layout metadata。

### PR-DOCINT-1：ParseMode 路由 + Docling adapter + 页面文本模型

- 在 ingestion parser 层建立显式解析模式路由：
  - `CHUNK` / `BASIC`：走 Tika，保持现有普通 RAG 入库行为；
  - `PIPELINE` / `ENHANCED`：走 Docling，保留页码、标题路径、表格、bbox 等 layout 信息；
  - Collateral KB：后端策略默认或强制选择 `PIPELINE` / `ENHANCED`。
- 预留质量检测接口，但 MVP 不启用自动检测：
  - `ParseModePolicy`：根据 KB 类型、用户选择、未来质量检测结果决定解析模式；
  - `TextLayerQualityDetector`：后续用于 text density / image-only / mixed page 判断；
  - `OcrFallbackParser`：后续用于扫描件 OCR fallback。
- 新增统一页面文本模型：
...
LayoutBlock
  blockId
  pageNo
  blockType: TITLE | PARAGRAPH | TABLE | HEADER | FOOTER | LIST | CAPTION | OTHER
  text
...
- `DocumentPageText` 是后续 page preview 和字段证据定位的页级契约，入库后需要可持久化或等价缓存；不能只作为一次性 parser 内存对象存在。
- Docling 通过 adapter 接入，不把具体解析引擎锁进业务 service。
- 扫描件 OCR、云 OCR、mixed PDF 按页 fallback、复杂表格结构恢复留后续增强。
...
- `CHUNK` / BASIC 入库仍走现有 Tika 路径，不回归；
- `PIPELINE` / ENHANCED 入库能通过 Docling 产生结构化文本、表格、页码和 layout block；
- Collateral KB 文档入库默认或强制进入 `PIPELINE` / ENHANCED；
- 页级 `DocumentPageText` 可在入库后复用，支撑后续 page preview；
- chunk 能追溯到 pageNo、blockId，必要时追溯到 bbox；
- 页眉页脚不会主导检索结果；
- image-only / scanned PDF 不在 MVP 质量承诺内，但抽象接口已预留；
...
- 如果真实协议主要是双栏/表格等复杂版式 PDF，Phase 2.5 的 Docling layout 底座是 Phase 3 的硬前置。
- 如果真实协议包含扫描件、图片型 PDF，需先完成 Phase 2.5 后续 OCR fallback / text-layer quality detector 增强，再承诺字段抽取质量。
